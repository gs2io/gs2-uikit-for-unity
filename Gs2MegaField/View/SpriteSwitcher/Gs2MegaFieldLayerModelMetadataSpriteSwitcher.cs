    ...
    public partial class Gs2MegaFieldLayerModelMetadataSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.LayerModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyMetadatas.Contains(_fetcher.LayerModel.Metadata)) {
                    ...
                    case Expression.StartsWith:
                        if (applyMetadata.StartsWith(_fetcher.LayerModel.Metadata)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (applyMetadata.EndsWith(_fetcher.LayerModel.Metadata)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
