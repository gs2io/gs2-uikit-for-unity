...
{
    [AddComponentMenu("GS2 UIKit/Quest/QuestModel/Convert/Gs2QuestConvertQuestModelFirstCompleteAcquireActionsToAcquireActions")]
    public class Gs2QuestConvertQuestModelFirstCompleteAcquireActionsToAcquireActions : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.QuestModel != null && _callbackCount != count)
            {
                this.onConverted.Invoke(
                    _fetcher.QuestModel.FirstCompleteAcquireActions.Select(v => AcquireAction.New(
                        v.Action,
                        v.Request
                    ) * count).ToList()
                );
