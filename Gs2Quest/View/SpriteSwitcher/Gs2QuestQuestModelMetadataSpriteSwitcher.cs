    ...
    public partial class Gs2QuestQuestModelMetadataSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.QuestModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyMetadatas.Contains(_fetcher.QuestModel.Metadata)) {
                    ...
                    case Expression.StartsWith:
                        if (_fetcher.QuestModel.Metadata.StartsWith(applyMetadata)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (_fetcher.QuestModel.Metadata.EndsWith(applyMetadata)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
