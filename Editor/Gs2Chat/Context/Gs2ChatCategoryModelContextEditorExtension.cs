...
{
    [CustomEditor(typeof(Gs2ChatCategoryModelContext))]
    public class Gs2ChatCategoryModelContextEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2ChatCategoryModelContext;
            ...
            if (original.CategoryModel == null) {
                var list = original.GetComponentInParent<Gs2ChatCategoryModelList>(true);
                if (list != null) {
                    EditorGUILayout.HelpBox("CategoryModel is auto assign from Gs2ChatCategoryModelList.", MessageType.Info);
                    EditorGUI.BeginDisabledGroup(true);
                    EditorGUILayout.ObjectField("List", list, typeof(Gs2ChatCategoryModelList), false);
                ...
                if (original.CategoryModel != null) {
                    ...
                    EditorGUILayout.TextField("Category", original.CategoryModel?.Category.ToString());
                    EditorGUI.indentLevel--;
                }
                EditorGUI.EndDisabledGroup();
            }
