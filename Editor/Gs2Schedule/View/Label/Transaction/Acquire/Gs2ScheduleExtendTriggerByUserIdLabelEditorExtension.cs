...
{
    [CustomEditor(typeof(Gs2ScheduleExtendTriggerByUserIdLabel))]
    public class Gs2ScheduleExtendTriggerByUserIdLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2ScheduleExtendTriggerByUserIdLabel;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2ScheduleExtendTriggerByUserIdFetcher>() ?? original.GetComponentInParent<Gs2ScheduleExtendTriggerByUserIdFetcher>(true);
            ...
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2ScheduleExtendTriggerByUserIdFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add Fetcher")) {
                    original.gameObject.AddComponent<Gs2ScheduleExtendTriggerByUserIdFetcher>();
                }
            }
            ...
            if (GUILayout.Button("ExtendSeconds")) {
                original.format += "{extendSeconds}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
