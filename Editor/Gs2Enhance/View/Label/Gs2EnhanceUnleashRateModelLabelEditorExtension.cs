...
{
    [CustomEditor(typeof(Gs2EnhanceUnleashRateModelLabel))]
    public class Gs2EnhanceUnleashRateModelLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2EnhanceUnleashRateModelLabel;
            ...
            if (fetcher == null) {
                ...
                if (fetcher.gameObject.GetComponentInParent<Gs2EnhanceUnleashRateModelList>(true) != null) {
                    ...
                    if (fetcher.Context != null) {
                        EditorGUILayout.ObjectField("UnleashRateModel", fetcher.Context.UnleashRateModel, typeof(UnleashRateModel), false);
                        EditorGUI.indentLevel++;
                        EditorGUILayout.TextField("NamespaceName", fetcher.Context.UnleashRateModel?.NamespaceName?.ToString());
                        EditorGUILayout.TextField("RateName", fetcher.Context.UnleashRateModel?.RateName?.ToString());
                        EditorGUI.indentLevel--;
                    }
                    EditorGUI.indentLevel--;
                    EditorGUI.EndDisabledGroup();
