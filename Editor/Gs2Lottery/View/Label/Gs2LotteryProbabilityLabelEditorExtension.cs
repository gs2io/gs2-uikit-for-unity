...
namespace Gs2.Unity.UiKit.Gs2Lottery.Editor
{
    ...
    public class Gs2LotteryProbabilityLabelEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            ...
            if (fetcher == null) {
                ...
                if (fetcher.gameObject.GetComponentInParent<Gs2LotteryProbabilityList>(true) != null) {
            ...
            if (GUILayout.Button("Rate(0-1)")) {
                original.format += "{rate:0-1}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
            if (GUILayout.Button("Rate(0.0-100.0)")) {
                original.format += "{rate:0.0-100.0}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
            if (GUILayout.Button("Rate(0.00-100.00)")) {
                original.format += "{rate:0.00-100.00}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
            if (GUILayout.Button("Rate(0.000-100.000)")) {
                original.format += "{rate:0.000-100.000}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
            if (GUILayout.Button("Rate(0.0000-100.0000)")) {
                original.format += "{rate:0.0000-100.0000}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
            if (GUILayout.Button("Rate(0.00000-100.00000)")) {
                original.format += "{rate:0.00000-100.00000}";
                GUI.FocusControl("");
                EditorUtility.SetDirty(original);
            }
