...
{
    [CustomEditor(typeof(Gs2LotteryProbabilityList))]
    public class Gs2LotteryProbabilityListEditorExtension : UnityEditor.Editor
    {
        public override void OnInspectorGUI() {
            var original = target as Gs2LotteryProbabilityList;

            if (original == null) return;

            var fetcher = original.GetComponent<Gs2LotteryProbabilityListFetcher>() ?? original.GetComponentInParent<Gs2LotteryProbabilityListFetcher>(true);
            if (fetcher == null) {
                EditorGUILayout.HelpBox("Gs2LotteryProbabilityListFetcher not found.", MessageType.Error);
                if (GUILayout.Button("Add ListFetcher")) {
                    original.gameObject.AddComponent<Gs2LotteryProbabilityListFetcher>();
                }
            }
            else {
