...
namespace Gs2.Unity.UiKit.Gs2Formation
{
	...
	[AddComponentMenu("GS2 UIKit/Formation/PropertyForm/View/Properties/Name/Gs2FormationPropertyFormNameEnabler")]
            ...
            if (_fetcher.Fetched && _fetcher.PropertyForm != null)
            {
                switch(expression)
                {
                    ...
                    case Expression.StartsWith:
                        target.SetActive(enableName.StartsWith(_fetcher.PropertyForm.Name));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(enableName.EndsWith(_fetcher.PropertyForm.Name));
                        break;
                }
            }
            else
