    ...
    public partial class Gs2FormationPropertyFormModelMetadataEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.PropertyFormModel != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableMetadatas.Contains(_fetcher.PropertyFormModel.Metadata));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableMetadatas.Contains(_fetcher.PropertyFormModel.Metadata));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.PropertyFormModel.Metadata.StartsWith(enableMetadata));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.PropertyFormModel.Metadata.EndsWith(enableMetadata));
                        break;
                }
            }
            else
