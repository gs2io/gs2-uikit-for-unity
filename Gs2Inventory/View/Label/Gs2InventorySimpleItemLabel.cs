...
namespace Gs2.Unity.UiKit.Gs2Inventory
{
	...
	[AddComponentMenu("GS2 UIKit/Inventory/SimpleItem/View/Label/Gs2InventorySimpleItemLabel")]
    public partial class Gs2InventorySimpleItemLabel : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.SimpleItem != null)
            {
                onUpdate?.Invoke(
                    format.Replace(
                        "{itemId}", $"{_fetcher?.SimpleItem?.ItemId}"
                    ).Replace(
                        "{itemName}", $"{_fetcher?.SimpleItem?.ItemName}"
                    ).Replace(
                        "{count}", $"{_fetcher?.SimpleItem?.Count}"
                    )
                );
            }
        }
    ...
    public partial class Gs2InventorySimpleItemLabel
    {
