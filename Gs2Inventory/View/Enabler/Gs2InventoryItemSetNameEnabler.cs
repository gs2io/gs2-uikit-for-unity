...
namespace Gs2.Unity.UiKit.Gs2Inventory.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Inventory/ItemSet/View/Enabler/Properties/Name/Gs2InventoryItemSetNameEnabler")]
            ...
            if (_fetcher.Fetched && _fetcher.ItemSet != null)
            {
                switch(expression)
                {
                    ...
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.ItemSet[index].Name.StartsWith(enableName));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.ItemSet[index].Name.EndsWith(enableName));
                        break;
                }
            }
            else
