    ...
    public partial class Gs2InventoryBigInventoryModelMetadataEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.BigInventoryModel != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableMetadatas.Contains(_fetcher.BigInventoryModel.Metadata));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableMetadatas.Contains(_fetcher.BigInventoryModel.Metadata));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.BigInventoryModel.Metadata.StartsWith(enableMetadata));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.BigInventoryModel.Metadata.EndsWith(enableMetadata));
                        break;
                }
            }
            else
