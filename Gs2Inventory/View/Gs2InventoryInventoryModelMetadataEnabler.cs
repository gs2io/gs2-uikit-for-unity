...
namespace Gs2.Unity.UiKit.Gs2Inventory
{
	...
	[AddComponentMenu("GS2 UIKit/Inventory/InventoryModel/View/Properties/Metadata/Gs2InventoryInventoryModelMetadataEnabler")]
    public partial class Gs2InventoryInventoryModelMetadataEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.InventoryModel != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableMetadatas.Contains(_fetcher.InventoryModel.Metadata));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableMetadatas.Contains(_fetcher.InventoryModel.Metadata));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(enableMetadata.StartsWith(_fetcher.InventoryModel.Metadata));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(enableMetadata.EndsWith(_fetcher.InventoryModel.Metadata));
                        break;
                }
            }
            else
