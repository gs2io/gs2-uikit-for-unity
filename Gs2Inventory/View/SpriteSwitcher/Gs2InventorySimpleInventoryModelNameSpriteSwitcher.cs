    ...
    public partial class Gs2InventorySimpleInventoryModelNameSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.SimpleInventoryModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyNames.Contains(_fetcher.SimpleInventoryModel.Name)) {
                    ...
                    case Expression.StartsWith:
                        if (applyName.StartsWith(_fetcher.SimpleInventoryModel.Name)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (applyName.EndsWith(_fetcher.SimpleInventoryModel.Name)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
