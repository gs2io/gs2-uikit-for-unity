...
namespace Gs2.Unity.UiKit.Gs2Account
{
	...
	[AddComponentMenu("GS2 UIKit/Account/TakeOver/View/Label/Gs2AccountTakeOverLabel")]
    public partial class Gs2AccountTakeOverLabel : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.TakeOver != null)
            {
                ...
                onUpdate?.Invoke(
                    format.Replace(
                        "{userId}", $"{_fetcher?.TakeOver?.UserId}"
                    ).Replace(
                        "{type}", $"{_fetcher?.TakeOver?.Type}"
                    ).Replace(
                        "{userIdentifier}", $"{_fetcher?.TakeOver?.UserIdentifier}"
                    ).Replace(
