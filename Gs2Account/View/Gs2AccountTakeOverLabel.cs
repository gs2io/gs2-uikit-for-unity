...
namespace Gs2.Unity.UiKit.Gs2Account
{
	...
	[AddComponentMenu("GS2 UIKit/Account/TakeOver/View/Gs2AccountTakeOverLabel")]
    public partial class Gs2AccountTakeOverLabel : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched)
            {
                var createdAt = UnixTime.FromUnixTime(_fetcher.TakeOver.CreatedAt).ToLocalTime();
                onUpdate.Invoke(
                    format.Replace(
                        "{userId}", _fetcher.TakeOver.UserId.ToString()
                    ).Replace(
                        "{type}", _fetcher.TakeOver.Type.ToString()
                    ).Replace(
                        "{userIdentifier}", _fetcher.TakeOver.UserIdentifier.ToString()
                    ).Replace(
