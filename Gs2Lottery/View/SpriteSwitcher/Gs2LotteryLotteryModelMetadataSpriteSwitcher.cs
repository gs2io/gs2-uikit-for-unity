    ...
    public partial class Gs2LotteryLotteryModelMetadataSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.LotteryModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyMetadatas.Contains(_fetcher.LotteryModel.Metadata)) {
                    ...
                    case Expression.StartsWith:
                        if (applyMetadata.StartsWith(_fetcher.LotteryModel.Metadata)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (applyMetadata.EndsWith(_fetcher.LotteryModel.Metadata)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
