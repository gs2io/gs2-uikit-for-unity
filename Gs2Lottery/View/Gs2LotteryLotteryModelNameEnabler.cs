...
namespace Gs2.Unity.UiKit.Gs2Lottery
{
	...
	[AddComponentMenu("GS2 UIKit/Lottery/LotteryModel/View/Properties/Name/Gs2LotteryLotteryModelNameEnabler")]
            ...
            if (_fetcher.Fetched && _fetcher.LotteryModel != null)
            {
                switch(expression)
                {
                    ...
                    case Expression.StartsWith:
                        target.SetActive(enableName.StartsWith(_fetcher.LotteryModel.Name));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(enableName.EndsWith(_fetcher.LotteryModel.Name));
                        break;
                }
            }
            else
