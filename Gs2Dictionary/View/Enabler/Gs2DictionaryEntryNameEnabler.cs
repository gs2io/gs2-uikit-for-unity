...
namespace Gs2.Unity.UiKit.Gs2Dictionary.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Dictionary/Entry/View/Enabler/Properties/Name/Gs2DictionaryEntryNameEnabler")]
            ...
            if (_fetcher.Fetched && _fetcher.Entry != null)
            {
                switch(expression)
                {
                    ...
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.Entry.Name.StartsWith(enableName));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.Entry.Name.EndsWith(enableName));
                        break;
                }
            }
            else
