...
namespace Gs2.Unity.UiKit.Gs2Dictionary
{
	...
	[AddComponentMenu("GS2 UIKit/Dictionary/Like/View/Label/Gs2DictionaryOwnLikeLabel")]
    public partial class Gs2DictionaryOwnLikeLabel : MonoBehaviour
    {
        private void OnFetched()
        {
            this.onUpdate?.Invoke(
                this.format.Replace(
                    "{likeId}", $"{this._fetcher?.Like?.LikeId}"
                ).Replace(
                    "{userId}", $"{this._fetcher?.Like?.UserId}"
                ).Replace(
                    "{name}", $"{this._fetcher?.Like?.Name}"
                )
            );
        }
    }
    ...
    public partial class Gs2DictionaryOwnLikeLabel
    {
