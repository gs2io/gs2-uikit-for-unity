...
namespace Gs2.Unity.UiKit.Gs2Exchange.SpriteSwitcher
{
	...
	[AddComponentMenu("GS2 UIKit/Exchange/Await/View/SpriteSwitcher/Properties/UserId/Gs2ExchangeAwaitUserIdSpriteTableSwitcher")]
    public partial class Gs2ExchangeAwaitUserIdSpriteTableSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.Await != null)
            {
                if (sprites.Count(v => v.value == _fetcher.Await.UserId) > 0) {
                    this.onUpdate.Invoke(sprites.Find(v => v.value == _fetcher.Await.UserId).sprite);
                }
                else {
    ...
    public partial class Gs2ExchangeAwaitUserIdSpriteTableSwitcher
    {
