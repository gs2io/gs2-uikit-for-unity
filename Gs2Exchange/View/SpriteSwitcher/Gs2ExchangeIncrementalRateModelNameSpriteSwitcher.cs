    ...
    public partial class Gs2ExchangeIncrementalRateModelNameSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.IncrementalRateModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyNames.Contains(_fetcher.IncrementalRateModel.Name)) {
                    ...
                    case Expression.StartsWith:
                        if (_fetcher.IncrementalRateModel.Name.StartsWith(applyName)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (_fetcher.IncrementalRateModel.Name.EndsWith(applyName)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
