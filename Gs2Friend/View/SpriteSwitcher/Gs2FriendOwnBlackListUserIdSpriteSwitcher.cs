    ...
    public partial class Gs2FriendOwnBlackListUserIdSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.BlackList != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyUserIds.Contains(_fetcher.BlackList.UserId)) {
                    ...
                    case Expression.StartsWith:
                        if (_fetcher.BlackList.UserId.StartsWith(applyUserId)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (_fetcher.BlackList.UserId.EndsWith(applyUserId)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
