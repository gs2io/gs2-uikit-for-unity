    ...
    public partial class Gs2SerialKeyCampaignModelNameSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.CampaignModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyNames.Contains(_fetcher.CampaignModel.Name)) {
                    ...
                    case Expression.StartsWith:
                        if (applyName.StartsWith(_fetcher.CampaignModel.Name)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (applyName.EndsWith(_fetcher.CampaignModel.Name)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
