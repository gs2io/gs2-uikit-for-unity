...
namespace Gs2.Unity.UiKit.Gs2SerialKey
{
	...
	[AddComponentMenu("GS2 UIKit/SerialKey/SerialKey/View/Properties/Status/Gs2SerialKeySerialKeyStatusEnabler")]
    public partial class Gs2SerialKeySerialKeyStatusEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.SerialKey != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableStatuses.Contains(_fetcher.SerialKey.Status));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableStatuses.Contains(_fetcher.SerialKey.Status));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(enableStatus.StartsWith(_fetcher.SerialKey.Status));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(enableStatus.EndsWith(_fetcher.SerialKey.Status));
                        break;
                }
            }
            else
