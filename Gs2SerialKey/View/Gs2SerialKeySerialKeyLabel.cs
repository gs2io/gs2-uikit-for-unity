...
namespace Gs2.Unity.UiKit.Gs2SerialKey
{
	...
	[AddComponentMenu("GS2 UIKit/SerialKey/SerialKey/View/Gs2SerialKeySerialKeyLabel")]
    public partial class Gs2SerialKeySerialKeyLabel : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched)
            {
                onUpdate.Invoke(
                    format.Replace(
                        "{campaignModelName}", _fetcher.SerialKey.CampaignModelName.ToString()
                    ).Replace(
                        "{metadata}", _fetcher.SerialKey.Metadata.ToString()
                    ).Replace(
                        "{code}", _fetcher.SerialKey.Code.ToString()
                    ).Replace(
                        "{status}", _fetcher.SerialKey.Status.ToString()
                    )
                );
            }
        }
