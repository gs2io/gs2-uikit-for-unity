    ...
    public partial class Gs2MissionMissionTaskModelMetadataSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.MissionTaskModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyMetadatas.Contains(_fetcher.MissionTaskModel.Metadata)) {
                    ...
                    case Expression.StartsWith:
                        if (_fetcher.MissionTaskModel.Metadata.StartsWith(applyMetadata)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (_fetcher.MissionTaskModel.Metadata.EndsWith(applyMetadata)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
