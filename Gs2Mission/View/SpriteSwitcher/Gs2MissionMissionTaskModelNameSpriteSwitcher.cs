    ...
    public partial class Gs2MissionMissionTaskModelNameSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.MissionTaskModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyNames.Contains(_fetcher.MissionTaskModel.Name)) {
                    ...
                    case Expression.StartsWith:
                        if (applyName.StartsWith(_fetcher.MissionTaskModel.Name)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (applyName.EndsWith(_fetcher.MissionTaskModel.Name)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
