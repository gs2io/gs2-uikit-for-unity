    ...
    public partial class Gs2LoginRewardBonusModelRepeatSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.BonusModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyRepeats.Contains(_fetcher.BonusModel.Repeat)) {
                    ...
                    case Expression.StartsWith:
                        if (_fetcher.BonusModel.Repeat.StartsWith(applyRepeat)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (_fetcher.BonusModel.Repeat.EndsWith(applyRepeat)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
