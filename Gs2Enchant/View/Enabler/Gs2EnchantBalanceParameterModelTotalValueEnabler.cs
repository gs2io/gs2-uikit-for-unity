    ...
    public partial class Gs2EnchantBalanceParameterModelTotalValueEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.BalanceParameterModel != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableTotalValues.Contains(_fetcher.BalanceParameterModel.TotalValue));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableTotalValues.Contains(_fetcher.BalanceParameterModel.TotalValue));
                        break;
                    case Expression.Less:
                        target.SetActive(enableTotalValue > _fetcher.BalanceParameterModel.TotalValue);
                        break;
