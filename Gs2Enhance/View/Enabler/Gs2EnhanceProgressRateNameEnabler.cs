...
namespace Gs2.Unity.UiKit.Gs2Enhance.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Enhance/Progress/View/Enabler/Properties/RateName/Gs2EnhanceProgressRateNameEnabler")]
            ...
            if (_fetcher.Fetched && _fetcher.Progress != null)
            {
                switch(expression)
                {
                    ...
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.Progress.RateName.StartsWith(enableRateName));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.Progress.RateName.EndsWith(enableRateName));
                        break;
                }
            }
            else
