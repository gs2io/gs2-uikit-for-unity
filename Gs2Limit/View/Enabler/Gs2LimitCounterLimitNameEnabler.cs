...
namespace Gs2.Unity.UiKit.Gs2Limit.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Limit/Counter/View/Enabler/Properties/LimitName/Gs2LimitCounterLimitNameEnabler")]
            ...
            if (_fetcher.Fetched && _fetcher.Counter != null)
            {
                switch(expression)
                {
                    ...
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.Counter.LimitName.StartsWith(enableLimitName));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.Counter.LimitName.EndsWith(enableLimitName));
                        break;
                }
            }
            else
