...
namespace Gs2.Unity.UiKit.Gs2Matchmaking.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Matchmaking/Rating/View/Enabler/Properties/Name/Gs2MatchmakingRatingNameEnabler")]
    public partial class Gs2MatchmakingRatingNameEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.Rating != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableNames.Contains(_fetcher.Rating.Name));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableNames.Contains(_fetcher.Rating.Name));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.Rating.Name.StartsWith(enableName));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.Rating.Name.EndsWith(enableName));
                        break;
                }
            }
            else
    ...
    public partial class Gs2MatchmakingRatingNameEnabler
    {
