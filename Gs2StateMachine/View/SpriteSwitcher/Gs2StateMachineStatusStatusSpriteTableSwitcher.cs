...
namespace Gs2.Unity.UiKit.Gs2StateMachine.SpriteSwitcher
{
	...
	[AddComponentMenu("GS2 UIKit/StateMachine/Status/View/SpriteSwitcher/Properties/Status/Gs2StateMachineStatusStatusSpriteTableSwitcher")]
    public partial class Gs2StateMachineStatusStatusSpriteTableSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.Status != null)
            {
                if (sprites.Count(v => v.value == _fetcher.Status.Status) > 0) {
                    this.onUpdate.Invoke(sprites.Find(v => v.value == _fetcher.Status.Status).sprite);
                }
                else {
    ...
    public partial class Gs2StateMachineStatusStatusSpriteTableSwitcher
    {
