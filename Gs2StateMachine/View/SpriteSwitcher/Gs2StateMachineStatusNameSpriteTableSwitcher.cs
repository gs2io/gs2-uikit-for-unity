...
namespace Gs2.Unity.UiKit.Gs2StateMachine.SpriteSwitcher
{
	...
	[AddComponentMenu("GS2 UIKit/StateMachine/Status/View/SpriteSwitcher/Properties/Name/Gs2StateMachineStatusNameSpriteTableSwitcher")]
    public partial class Gs2StateMachineStatusNameSpriteTableSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.Status != null)
            {
                if (sprites.Count(v => v.value == _fetcher.Status.Name) > 0) {
                    this.onUpdate.Invoke(sprites.Find(v => v.value == _fetcher.Status.Name).sprite);
                }
                else {
    ...
    public partial class Gs2StateMachineStatusNameSpriteTableSwitcher
    {
