...
namespace Gs2.Unity.UiKit.Gs2Idle.SpriteSwitcher
{
	...
	[AddComponentMenu("GS2 UIKit/Idle/Status/View/SpriteSwitcher/Properties/CategoryName/Gs2IdleStatusCategoryNameSpriteSwitcher")]
    public partial class Gs2IdleStatusCategoryNameSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.Status != null)
            {
                switch(expression)
                {
                    ...
                    case Expression.StartsWith:
                        if (applyCategoryName.StartsWith(_fetcher.Status.CategoryName)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (applyCategoryName.EndsWith(_fetcher.Status.CategoryName)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
    ...
    public partial class Gs2IdleStatusCategoryNameSpriteSwitcher
    {
