    ...
    public partial class Gs2RankingCategoryModelNameSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.CategoryModel != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyNames.Contains(_fetcher.CategoryModel.Name)) {
                    ...
                    case Expression.StartsWith:
                        if (applyName.StartsWith(_fetcher.CategoryModel.Name)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (applyName.EndsWith(_fetcher.CategoryModel.Name)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
