...
namespace Gs2.Unity.UiKit.Gs2Ranking.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Ranking/Score/View/Enabler/Properties/UniqueId/Gs2RankingScoreUniqueIdEnabler")]
            ...
            if (_fetcher.Fetched && _fetcher.Score != null)
            {
                switch(expression)
                {
                    ...
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.Score.UniqueId.StartsWith(enableUniqueId));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.Score.UniqueId.EndsWith(enableUniqueId));
                        break;
                }
            }
            else
