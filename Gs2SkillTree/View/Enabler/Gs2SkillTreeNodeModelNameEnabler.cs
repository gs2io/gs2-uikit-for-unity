    ...
    public partial class Gs2SkillTreeNodeModelNameEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.NodeModel != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableNames.Contains(_fetcher.NodeModel.Name));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableNames.Contains(_fetcher.NodeModel.Name));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(_fetcher.NodeModel.Name.StartsWith(enableName));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(_fetcher.NodeModel.Name.EndsWith(enableName));
                        break;
                }
            }
            else
