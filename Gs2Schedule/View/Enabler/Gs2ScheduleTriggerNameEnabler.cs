...
namespace Gs2.Unity.UiKit.Gs2Schedule.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Schedule/Trigger/View/Enabler/Properties/Name/Gs2ScheduleTriggerNameEnabler")]
    public partial class Gs2ScheduleTriggerNameEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.Trigger != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableNames.Contains(_fetcher.Trigger.Name));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableNames.Contains(_fetcher.Trigger.Name));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(enableName.StartsWith(_fetcher.Trigger.Name));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(enableName.EndsWith(_fetcher.Trigger.Name));
                        break;
                }
            }
            else
    ...
    public partial class Gs2ScheduleTriggerNameEnabler
    {
