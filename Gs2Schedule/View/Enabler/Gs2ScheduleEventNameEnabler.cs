...
namespace Gs2.Unity.UiKit.Gs2Schedule.Enabler
{
	...
	[AddComponentMenu("GS2 UIKit/Schedule/Event/View/Enabler/Properties/Name/Gs2ScheduleEventNameEnabler")]
    public partial class Gs2ScheduleEventNameEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.Event != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableNames.Contains(_fetcher.Event.Name));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableNames.Contains(_fetcher.Event.Name));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(enableName.StartsWith(_fetcher.Event.Name));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(enableName.EndsWith(_fetcher.Event.Name));
                        break;
                }
            }
            else
    ...
    public partial class Gs2ScheduleEventNameEnabler
    {
