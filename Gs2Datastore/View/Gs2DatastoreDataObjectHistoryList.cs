...
namespace Gs2.Unity.UiKit.Gs2Datastore
{
        ...
        public void OnFetched() {
            for (var i = 0; i < this._children.Count; i++) {
                if (i < this._fetcher.DataObjectHistories.Count) {
                    this._children[i].SetDataObjectHistory(
                        DataObjectHistory.New(
    ...
    public partial class Gs2DatastoreDataObjectHistoryList
    {
        ...
        {
            if (this.prefab == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2DatastoreDataObjectHistoryContext Prefab.");
                enabled = false;
                return;
            }

            this._fetcher = GetComponent<Gs2DatastoreDataObjectHistoryListFetcher>() ?? GetComponentInParent<Gs2DatastoreDataObjectHistoryListFetcher>();
            if (this._fetcher == null) {
                Debug.LogError($"{gameObject.GetFullPath()}: Couldn't find the Gs2DatastoreDataObjectHistoryListFetcher.");
                enabled = false;
            }

            this._children = new List<Gs2DatastoreDataObjectHistoryContext>();
        ...
        public virtual bool HasError()
        {
            ...
            this._fetcher = GetComponent<Gs2DatastoreDataObjectHistoryListFetcher>() ?? GetComponentInParent<Gs2DatastoreDataObjectHistoryListFetcher>(true);
            if (this._fetcher == null) {
                return true;
            }
            return false;
