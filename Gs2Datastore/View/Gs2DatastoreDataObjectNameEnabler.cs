...
namespace Gs2.Unity.UiKit.Gs2Datastore
{
	...
	[AddComponentMenu("GS2 UIKit/Datastore/DataObject/View/Properties/Name/Gs2DatastoreDataObjectNameEnabler")]
    public partial class Gs2DatastoreDataObjectNameEnabler : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.DataObject != null)
            {
                ...
                {
                    case Expression.In:
                        target.SetActive(enableNames.Contains(_fetcher.DataObject.Name));
                        break;
                    case Expression.NotIn:
                        target.SetActive(!enableNames.Contains(_fetcher.DataObject.Name));
                        break;
                    case Expression.StartsWith:
                        target.SetActive(enableName.StartsWith(_fetcher.DataObject.Name));
                        break;
                    case Expression.EndsWith:
                        target.SetActive(enableName.EndsWith(_fetcher.DataObject.Name));
                        break;
                }
            }
            else
