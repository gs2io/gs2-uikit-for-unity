    ...
    public partial class Gs2DatastoreDataObjectUserIdSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.DataObject != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyUserIds.Contains(_fetcher.DataObject.UserId)) {
                    ...
                    case Expression.StartsWith:
                        if (applyUserId.StartsWith(_fetcher.DataObject.UserId)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (applyUserId.EndsWith(_fetcher.DataObject.UserId)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
