    ...
    public partial class Gs2DatastoreDataObjectStatusSpriteSwitcher : MonoBehaviour
    {
        public void Update()
        {
            if (_fetcher.Fetched && _fetcher.DataObject != null)
            {
                ...
                {
                    case Expression.In:
                        if (applyStatuses.Contains(_fetcher.DataObject.Status)) {
                    ...
                    case Expression.StartsWith:
                        if (_fetcher.DataObject.Status.StartsWith(applyStatus)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                    case Expression.EndsWith:
                        if (_fetcher.DataObject.Status.EndsWith(applyStatus)) {
                            this.onUpdate.Invoke(this.sprite);
                        }
                        break;
                }
